...
export class CognitoStrategy extends PassportStrategy(Strategy, 'cognito') {
  ...
  async validate(@Req() req: Request) {
    ...
    const config = AWSutil.cognito.config({
      ...
      client_secret: this._configService.get<string>(
        'ARIM_COGNITO_CLIENT_SECRET',
      ),
      code: code,
      redirect_uri: this._configService.get<string>(
        'ARIM_COGNITO_REDIRECT_URI',
      ),
    });
  ...
  private _createToken(config: ResponseCognitoDto): Promise<string> {
    return firstValueFrom(
      this._http
        .post(
          `${this._configService.get<string>('ARIM_COGNITO_LINK')}/oauth2/token`,
          config.body,
          config.headers,
        )
        .pipe(
  ...
  private _getProfileData(accessToken: string): Promise<CognitoProfileDto> {
    return firstValueFrom(
      this._http
        .get(
          `${this._configService.get<string>('ARIM_COGNITO_LINK')}/oauth2/userInfo`,
          {
            headers: {
